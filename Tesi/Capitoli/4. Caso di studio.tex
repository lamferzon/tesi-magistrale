...
\begin{itemize}
	\item variabili meteorologiche $\mathbf{x}_{meteo}(t)$;
	\item variabili spaziali $\mathbf{x}_{spazio}(\mathbf{s})$;
	\item variabili dummy $\mathbf{x}_{dummy}(t)$.
\end{itemize}
Per completezza:
\begin{itemize}
...
\par L'andamento orario del numero medio di noleggi in figura~\ref{ritiri_orari} conferma quanto detto precedentemente. Da sottolineare, inoltre, la presenza di picchi di utilizzo in concomitanza delle ore di punta.
...
\begin{figure}[htpb]
	\centering
	\includegraphics[height=180px]{Immagini/4. Caso di studio/Serie storiche/Ritiri orari}
	\caption[Numero medio di ritiri al variare della stagione]{andamento orario del numero medio di ritiri al variare della stagione.}
	\label{ritiri_orari}
\end{figure}
